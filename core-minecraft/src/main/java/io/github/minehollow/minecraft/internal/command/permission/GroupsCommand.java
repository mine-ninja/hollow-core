...
public class GroupsCommand extends SimpleCommand {
    ...
    private void handleAddGroupCommand(@NotNull CommandContext ctx, @NotNull String playerName) {
        var group = ctx.getArgOrThrow(2, PlayerGroup.BY_ID::get, "§cGrupo inválido. Use um dos seguintes: " + String.join(", ", PlayerGroup.NAMES));
...
//        }
        ...
        final var categoryType = SubscriptionCategoryType.GLOBAL;
        playerAccountService.getPlayerAccountByName(playerName).whenComplete((account, error) -> {
            ...
            var playerAccount = (isPermanent || parsedTime == null) ?
                account.withNewPermanentSubscription(group, categoryType) : account.withNewSubscription(group, parsedTime, categoryType);

            playerAccountService.updatePlayerAccount(playerAccount).whenComplete((updatedAccount, updateError) -> {
                if (updateError != null) {
            ...
            .whenComplete((account, error) -> {
                ...
                playerAccountService.updatePlayerAccount(account.removeSubscription(group, categoryType))
                    .whenComplete((updatedAccount, updateError) -> {
                        ...
                        ctx.sendMessage("§aGrupo %s§aremovido do jogador %s com sucesso.".formatted(group.getPrefix(), playerName));
                    });
            });
    }
            ...
            .whenComplete((account, error) -> {
                ...
                for (var category : SubscriptionCategoryType.BY_NAME.values()) {
                    ...
                    for (var subscription : categorySubscriptions) {
                        ...
                        if (isPermanent || group == PlayerGroup.DEFAULT) {
