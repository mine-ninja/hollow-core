...
import io.github.minehollow.minecraft.menu.input.SignInputContext;
import io.github.minehollow.minecraft.menu.input.SignInputMenu;
import io.github.minehollow.minecraft.menu.item.SimpleMenuItem;
import io.github.minehollow.minecraft.util.inventory.InventoryUpdate;
...
public class PlayerMenuContext implements MenuContext {
    ...
    @Override
    public void update() {
        if(inventory == null) throw new IllegalStateException("Inventory not initialized for this context.");
    ...
    public void open() {
        ...
        items.forEach(
          (slot, menuItem) -> inventory.setItem(slot, menuItem.renderer().apply(player))
        );
