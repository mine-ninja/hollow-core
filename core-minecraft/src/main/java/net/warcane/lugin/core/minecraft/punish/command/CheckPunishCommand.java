...
public class CheckPunishCommand extends SimpleCommand {
    ...
    private void showCheckPunish(Player player, PunishedDTO punished) {
        ...
        for (PunishedDTO.Punishment punishment : punished.getPunishments()) {
            PunishmentInfo info = PunishmentInfo.getPunishmentById(punishment.getPunishmentInfoId());
            ...
            Tuple<PunishTime, PunishmentType> punishmentTimeType = info.getPunishment(punishment.getRepeatCount());
            ...
            if (punishment.getRevokerUuid() == null) {
                msg.actionHover(" <l-white>[Revogar]", (audience -> player.performCommand("revogar " + punishment.getId())), "<l-gray>Clique para revogar esta punição.");
            }
            msg.newLine();
        }
        msg.newLine();
