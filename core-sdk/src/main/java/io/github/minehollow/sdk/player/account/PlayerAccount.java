package io.github.minehollow.sdk.player.account;
...
import io.github.minehollow.sdk.player.account.data.ScopedData;
import io.github.minehollow.sdk.player.permissions.PermissionGraph;
import io.github.minehollow.sdk.player.permissions.PlayerPermission;
...
import static io.github.minehollow.sdk.player.subscription.PlayerGroupSubscription.createNewPermanentSubscription;
import static io.github.minehollow.sdk.player.subscription.PlayerGroupSubscription.createNewSubscription;
...
public record PlayerAccount(
    ...
    public Component getFormattedDisplayNameComponent(@NotNull SubscriptionCategoryType type, @Nullable Component betweenComp) {
        final var currentSubscription = this.getHighestSubscription(type);
        final var primaryGroup = currentSubscription.group();
        final Component groupPrefix = Component.text(primaryGroup.getModernTag() == ' ' ? "" : primaryGroup.getModernTag() + " ").style(Style.style().font(Key.key("hollow:tags")).color(TextColor.color(0xFFFFFF)).build());
