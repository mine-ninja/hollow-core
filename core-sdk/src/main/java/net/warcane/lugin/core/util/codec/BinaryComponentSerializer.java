...
public final class BinaryComponentSerializer {
    ...
    // region [Serialize]
    public static byte[] serializeUnchecked(Component value) {
        try {
            return serialize(value);
        } catch (IOException e) {
            throw new UncheckedIOException("Failed to serialize Component to bytes", e);
        }
    }

    public static byte[] serialize(Component value) throws IOException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        serializeComponent(value, new DataOutputStream(baos));
        return baos.toByteArray();
    }

    private static void serializeComponent(Component value, DataOutputStream output) throws IOException {
        serializeComponent(value, output, true);
    }

    private static void serializeComponent(Component value, DataOutputStream output, boolean header)
            throws IOException {
        if (header) {
    ...
    private static void serializeOptional(Component value, DataOutputStream output) throws IOException {
        if (value != null) {
    ...
    private static void serializeStyle(Style value, DataOutputStream output) throws IOException {
    ...
    private static void serializeShowItem(HoverEvent.ShowItem value, DataOutputStream output) throws IOException {
    ...
    private static void serializeShowEntity(HoverEvent.ShowEntity value, DataOutputStream output) throws IOException {
    ...
    private static void serializeBlockNbtPos(BlockNBTComponent.Pos pos, DataOutputStream output) throws IOException {
    ...
    private static void serializeCoordinate(BlockNBTComponent.WorldPos.Coordinate coordinate, DataOutputStream output)
            throws IOException {
        serializeSignedInt(coordinate.value(), output);
        output.writeByte((byte) coordinate.type().ordinal());
    }

    private static void serializeKey(Key key, DataOutputStream output) throws IOException {
        serializeString(key.namespace(), output);
        serializeString(key.value(), output);
    }

    // endregion [Deserialize]
    public static Component deserializeUnchecked(byte[] bytes) {
        try {
            return deserialize(bytes);
        } catch (final IOException e) {
            throw new UncheckedIOException("Failed to deserialize byte array to Component", e);
        }
    }

    public static Component deserialize(byte[] bytes) throws IOException {
        ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
        return deserializeComponent(new DataInputStream(bais));
    }

    private static Component deserializeComponent(DataInputStream input) throws IOException {
        return deserializeComponent(input, true);
    }

    private static Component deserializeComponent(DataInputStream input, boolean header) throws IOException {
        if (header) {
        ...
        ComponentBuilder<?, ?> builder =
                switch (componentType) {
                    case COMPONENT_TEXT -> Component.text().content(deserializeString(input));
                    ...
                    case COMPONENT_KEYBIND -> Component.keybind().keybind(deserializeString(input));
    ...
    private static Style deserializeStyle(DataInputStream input) throws IOException {
        ...
        if ((state & STYLE_COLOR_MASK) != 0) {
            int color =
                    ((input.readByte() & 0xFF) << 16) | ((input.readByte() & 0xFF) << 8) | (input.readByte() & 0xFF);
        ...
        if ((state & STYLE_CLICK_EVENT_MASK) != 0) {
            ...
            builder.clickEvent(ClickEvent.clickEvent(ClickEvent.Action.values()[actionId], deserializeString(input)));
        }
    ...
    private static HoverEvent.ShowItem deserializeShowItem(DataInputStream input) throws IOException {
    ...
    private static HoverEvent.ShowEntity deserializeShowEntity(DataInputStream input) throws IOException {
    ...
    private static BlockNBTComponent.Pos deserializeBlockNbtPos(DataInputStream input) throws IOException {
        return switch (input.readByte()) {
            case 0 -> BlockNBTComponent.WorldPos.worldPos(
                    deserializeCoordinate(input), deserializeCoordinate(input), deserializeCoordinate(input));
            case 1 -> BlockNBTComponent.LocalPos.localPos(input.readDouble(), input.readDouble(), input.readDouble());
            default -> throw notSureHowToDeserialize();
        };
    }

    private static BlockNBTComponent.WorldPos.Coordinate deserializeCoordinate(DataInputStream input)
            throws IOException {
        return BlockNBTComponent.WorldPos.Coordinate.coordinate(
                deserializeSignedInt(input), BlockNBTComponent.WorldPos.Coordinate.Type.values()[input.readByte()]);
    }

    @SuppressWarnings("PatternValidation")
    private static Key deserializeKey(DataInputStream input) throws IOException {
        return Key.key(deserializeString(input), deserializeString(input));
    }

    private static void serializeString(String value, DataOutputStream output) throws IOException {
        serializeBytes(value.getBytes(StandardCharsets.UTF_8), output);
    }

    private static String deserializeString(DataInputStream input) throws IOException {
        return new String(deserializeBytes(input), StandardCharsets.UTF_8);
    }

    private static void serializeBytes(byte[] value, DataOutputStream output) throws IOException {
        serializeVarInt(value.length, output);
        output.write(value);
    }

    private static byte[] deserializeBytes(DataInputStream input) throws IOException {
    ...
    static void serializeVarInt(@Range(from = 0, to = Integer.MAX_VALUE) int value, DataOutputStream output)
            throws IOException {
        if ((value & (0xFFFFFFFF << 7)) == 0) {
            ...
            output.writeInt((value & 0x7F | 0x80) << 24
