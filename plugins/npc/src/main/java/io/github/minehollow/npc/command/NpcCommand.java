...
/**
 * Main NPC command with subcommands for creating, removing, configuring NPCs.
 */
public class NpcCommand extends SimpleCommand {

    private static final List<String> SUB_COMMANDS = List.of(
        "create", "remove", "tp", "skin", "scale",
        "hologram", "action", "list", "info", "clicktype", "save"
    );
    ...
    private void handleCreate(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(2)) throw fail("§cUso: /npc create <id>");
        String id = ctx.getRawArgOrThrow(1 , "§cID do NPC é obrigatório.");
        Player player = ctx.getSenderAsPlayer();

        if (registry().get(id) != null) throw fail("§cNPC '" + id + "' já existe.");
    ...
    private void handleRemove(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(2)) throw fail("§cUso: /npc remove <id>");
        String id = ctx.getRawArgOrThrow(1, "§cUso: /npc remove <id>");
        if (!registry().remove(id)) throw fail("§cNPC '" + id + "' não encontrado.");
        ctx.sendMessage("§aNPC §f" + id + " §aremovido.");
    }

    private void handleTp(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(2)) throw fail("§cUso: /npc tp <id>");
    ...
    private void handleSkin(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(3)) throw fail("§cUso: /npc skin <id> <player | value signature>");
        Npc npc = requireNpc(ctx.getRawArgOrThrow(1, "§cUso: /npc skin <id> <player | value signature>"));
        ...
        // Player name — try online first, then async Mojang lookup
        String nameArg = ctx.getRawArgOrThrow(2, "§cUso: /npc skin <id> <player>");
        Player online = org.bukkit.Bukkit.getPlayerExact(nameArg);

        if (online != null) {
            // Online player — grab skin immediately
            var profile = online.getPlayerProfile();
            var prop = profile.getProperties().stream()
        ...
        // Offline player — async Mojang API lookup
        ctx.sendMessage("§7Buscando skin de §f" + nameArg + "§7...");
        plugin.getSkinService().fetchByName(nameArg).thenAccept(data -> {
            if (data == null) {
                Tasks.runSync(() -> ctx.sendMessage("§cNão foi possível encontrar a skin de '" + nameArg + "'."));
                return;
            }
            Tasks.runSync(() -> {
    ...
    private void handleScale(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(3)) throw fail("§cUso: /npc scale <id> <valor>");
        ...
        if (scale < min || scale > max) throw fail("§cEscala deve estar entre " + min + " e " + max + ".");
        npc.setScale(scale);
        registry().markDirty();
    ...
    private void handleHologram(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(3)) throw fail("§cUso: /npc hologram <id> <set|add|remove|offset> [args]");
        ...
        switch (action) {
            case "add" -> {
                if (ctx.isArgsLengthLessThan(4)) throw fail("§cUso: /npc hologram <id> add <texto>");
                String text = joinArgs(ctx.getArgs(), 3);
                npc.addHologramLine(text);
                ctx.sendMessage("§aLinha adicionada ao holograma.");
            }
            case "set" -> {
                if (ctx.isArgsLengthLessThan(5)) throw fail("§cUso: /npc hologram <id> set <índice> <texto>");
            ...
            case "remove" -> {
                if (ctx.isArgsLengthLessThan(4)) throw fail("§cUso: /npc hologram <id> remove <índice>");
            ...
            case "offset" -> {
                if (ctx.isArgsLengthLessThan(4)) throw fail("§cUso: /npc hologram <id> offset <valor>");
                double offset;
                try { offset = Double.parseDouble(ctx.getRawArgOrThrow(3, "§cUso: /npc hologram <id> offset <valor>")); }
                catch (NumberFormatException e) { throw fail("§cValor inválido."); }
    ...
    private void handleAction(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(3)) throw fail("§cUso: /npc action <id> <add|clear> [args]");
        ...
        switch (action) {
            ...
            case "add" -> {
                if (ctx.isArgsLengthLessThan(4)) throw fail("§cUso: /npc action <id> add <COMMAND|MESSAGE|SOUND|BROADCAST> [args]");
                String type = ctx.getRawArgOrThrow(3, "§cUso: /npc action <id> add <COMMAND|MESSAGE|SOUND|BROADCAST> [args]").toUpperCase();
                switch (type) {
                    case "COMMAND" -> {
                        if (ctx.isArgsLengthLessThan(6)) throw fail("§cUso: /npc action <id> add COMMAND <CONSOLE|PLAYER> <comando>");
                        CommandAction.Executor executor;
                        try { executor = CommandAction.Executor.valueOf(ctx.getRawArgOrThrow(4, "§cUso: /npc action <id> add COMMAND <CONSOLE|PLAYER> <comando>").toUpperCase()); }
                        catch (IllegalArgumentException e) { throw fail("§cExecutor inválido. Use CONSOLE ou PLAYER."); }
                        String cmd = joinArgs(ctx.getArgs(), 5);
                        npc.addAction(new CommandAction(executor, cmd));
                        ctx.sendMessage("§aAção COMMAND adicionada.");
                    }
                    case "MESSAGE" -> {
                        if (ctx.isArgsLengthLessThan(5)) throw fail("§cUso: /npc action <id> add MESSAGE <mensagem>");
                        npc.addAction(new MessageAction(joinArgs(ctx.getArgs(), 4)));
                        ctx.sendMessage("§aAção MESSAGE adicionada.");
                    }
                    case "SOUND" -> {
                        if (ctx.isArgsLengthLessThan(5)) throw fail("§cUso: /npc action <id> add SOUND <som> [volume] [pitch]");
                        ...
                        if (sound == null) sound = Registry.SOUNDS.get(NamespacedKey.minecraft(soundStr.toLowerCase()));
                        if (sound == null) throw fail("§cSom inválido: " + soundStr);
                        float volume = ctx.getArgs().length > 5 ? Float.parseFloat(ctx.getRawArgOrThrow(5, "§cUso: /npc action <id> add SOUND <som> <volume>")) : 1.0f;
                        float pitch = ctx.getArgs().length > 6 ? Float.parseFloat(ctx.getRawArgOrThrow(6, "§cUso: /npc action <id> add SOUND <som> <pitch>")) : 1.0f;
                        npc.addAction(new SoundAction(sound, volume, pitch));
                        ctx.sendMessage("§aAção SOUND adicionada.");
                    }
                    case "BROADCAST" -> {
                        if (ctx.isArgsLengthLessThan(5)) throw fail("§cUso: /npc action <id> add BROADCAST <mensagem>");
    ...
    private void handleInfo(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(2)) throw fail("§cUso: /npc info <id>");
        ...
        ctx.sendMessage(
            ...
            "§7Skin: §f" + (npc.getSkinValue() != null ? "definida" : "nenhuma"),
    ...
    private void handleClickType(@NotNull CommandContext ctx) throws CommandFailedException {
        if (ctx.isArgsLengthLessThan(3)) throw fail("§cUso: /npc clicktype <id> <RIGHT|LEFT|SHIFT_RIGHT|SHIFT_LEFT|ANY>");
        Npc npc = requireNpc(ctx.getRawArgOrThrow(1, "§cUso: /npc clicktype <id> <tipo>"));
        NpcClickType type;
        try { type = NpcClickType.valueOf(ctx.getRawArgOrThrow(2, "§cUso: /npc clicktype <id> <tipo>").toUpperCase()); }
    ...
    private @NotNull Npc requireNpc(@NotNull String id) throws CommandFailedException {
        ...
        if (npc == null) throw fail("§cNPC '" + id + "' não encontrado.");
        return npc;
    }
    ...
    private void sendHelp(@NotNull CommandContext ctx) {
        ctx.sendMessage(
            ...
            "§f/npc skin <id> <player|value sig> §7— Define a skin",
    ...
    public @NotNull List<String> tabComplete(@NotNull CommandSender sender, @NotNull String alias, @NotNull String[] args) {
        ...
        if (args.length == 2) {
            ...
            if (List.of("remove", "tp", "skin", "scale", "hologram", "action", "info", "clicktype").contains(sub)) {
        ...
        if (args.length == 3) {
            return switch (args[0].toLowerCase()) {
                ...
                case "clicktype" -> StringUtil.copyPartialMatches(args[2],
