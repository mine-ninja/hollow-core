...
public class PreviewSubCommand extends SimpleSubCommand {
    private final KitService kitService;
    private final BukkitPlatform platform;

    public PreviewSubCommand(KitService kitService, BukkitPlatform platform) {
        super("preview");
    ...
    protected void performSubCommand(CommandContext ctx) throws CommandFailedException {
        if (ctx.getArgs().length < 2) {
            throw new CommandFailedException("Uso: /kit preview <nome>");
        }
        ...
        platform.getMenuManager().openToPlayer(ctx.getSenderAsPlayer(), KitPreviewMenu.class, Map.of("kit", kit));
    }
